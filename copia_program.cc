...
void afegir_classificacio(const Classificacio &c) {
    map<string, Resultat>::const_iterator it = c.consultar_classificacio().begin();

    for (;it != c.consultar_classificacio().end(); ++it) {
        int r = jugadors[(*it).first].consultar_ranking() - 1;
        jugadors[(*it).first].afegir_resultat((*it).second);
        ranking[r - 1].second += (*it).second.punts;

        while (r > 0 and ranking[r - 1].second < ranking[r].second) {
            jugadors[ranking[r - 1].first].actualitzar_ranking(r);
